...
| | |
|---|---|
| **5,886 lines** of Python across **32 tools**, plus a suite of **168 checks**, verified by mutation | `score.py`, `score2.py`, `run_all.py`, `holdout.py`, `control_c2.py`, `unmapped_check.py`, `shiftaware.py`, `corpus_ghsa.py`, ... |
...
| **224 commits** as read on 2026-09-18, all public (`git rev-list --count HEAD`) | every correction visible in history. The count is a reading with a date, not a live number: the commit that updates this line is itself the next commit, so an exact figure here is stale the moment it lands |
...
- **2,009 raw artefacts** published under `raw/` - every tracked file that is neither a run log nor the

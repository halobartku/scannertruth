...
| **`radar`** at `main` 2026-09-02 (`24c56f9`, docker image; and `67348ee`, engine shim) | **11 / 11** both | **0 / 17** (`24c56f9`: 34/34 ok, 7 missed, 8 no-rule, 2 unlocated; `67348ee` not run on corpus 2) | noise 39% (`24c56f9`) and 11% (`67348ee`), upper bounds; rows and method in [`RESULTS-scanners.md`](RESULTS-scanners.md) |
| **`radar`** at `main` 2026-09-04 (`fa81c25`, **both** engine shim and docker image) | **11 / 11** | **1 / 17** (36/36 ok per pass; 1 detected, 6 missed, 8 no-rule, 2 unlocated) | the one detection is `wormhole-sysvar`, at the fix sites the vendor pre-announced in `radar#32`. Shim and image agree on **all 274 location rows** and on the score, so this row does not depend on the shim; per case in [`RESULTS-corpus2.md`](RESULTS-corpus2.md) |

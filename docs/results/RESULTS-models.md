...
`mappings/model-classes.json` on 2026-09-24 21:49 CEST; re-run it rather than editing the table.
...
- **`claude-opus-5-5`, Claude Code harness, effort max: 10 strict / 13 lenient of 17**, from 14
  verdict-right, 2026-09-24. **This is the first frontier row over the whole corpus:** 102 of 102
  calls answered, 0 unusable, where the `claude-opus-5` row below had 118 limit messages and
  covered 14 cases. Same strict count as Opus 5, so on this corpus 5.5 is not shown to be better at
  naming the class; it is complete where 5 was not. The one that falls is `squads-account-matching`
  (named a membership check, a different mechanism). The three disputed: `solend-owner-checks`
  (named the missing binding between reserve and lending market, not an owner check),
  `metaplex-token-metadata` ("account substitution" without saying which accounts must match) and
  `solido-deposit-reserve-account` (the same "unchecked reserve account" phrase judged disputed for
  glm-5.3). Silent on both variants: three cases, among them `spl-stake-pool-mint-decimals`. Cost
  shown is the harness's notional USD on the owner's subscription, not money spent.
...
suppressed regime and is not true of the reasoning regimes: under the adjudication rule Opus 5.5
reaches ten of seventeen (thirteen lenient), Opus 5 ten of the fourteen cases it answered, and
glm-5.3 five of seventeen, on prompt v1 with no tuning.
...
| `model-claude-opus-5-5-cc-c2-2026-09-24` | claude-opus-5-5 | claude-code | 2 | effort max | complete | 14 / 17 seen | 10 / 13 | 0 | 0 | 61.745 |

...
| `legacy/normalise_runs.py` | turn a directory of per-run Radar or VaultLint artefacts into the findings file and run log the clock reads; moved to `tools/legacy/` on 2026-09-02, see its README | `python tools/legacy/normalise_runs.py --kind vaultlint --runs <dir> --out raw/<out>.json` | nothing. Produced `raw/c2-vaultlint-complete.json` and its log, which the clock still scores; superseded for new work by `scanner_spec.py --run` |

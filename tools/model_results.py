...
necessary-not-sufficient count: fires on every run of the vulnerable variant that answered, silent on
the fixed one; the class the model named still has to be adjudicated by hand in mappings/, exactly
as RESULTS-models.md says. Cost is the sum of `cost_usd` on the lines: real money for OpenRouter,
an API-price estimate paid by the subscription for claude-code, none for ollama and the Z.ai plan.
...
def summarise(path):
    ...
    return {
        ...
        "verdict_right": right, "cases_seen": len(by_case),
...
def table(rows):
    out = ["| directory | model | provider | corpus | regime | calls | verdict right (necessary, not sufficient) | unusable | reasoned | cost USD |",
           "|---|---|---|---|---|---|---|---|---|---|"]
    for r in rows:
        ...
        out.append(f"| `{r['dir']}` | {r['model']} | {r['provider']} | {r['corpus']} | {r['regime']}{flag} | {status} | "
                   f"{r['verdict_right']} / {r['cases_seen']} seen | {r['unusable']} | {r['thinking_calls']} | {r['cost']} |")
    return "\n".join(out)
...
def demo():
    ...
    assert s["verdict_right"] == 1, s          # only case a: fires on vulnerable, silent on fixed
    assert s["unusable"] == 1 and s["cost"] == 3.0 and s["regime"] == "allowed" and not s["complete"], s
